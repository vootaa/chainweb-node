...
import Pact.Types.Lang(_LDecimal, _LString)
...
chainweb216Test = do
  ...
  assertMuslExponentiation :: CommandResult Hash -> Assertion
  assertMuslExponentiation tx = do
    let msg = "musl exponentiation regression"
    case tx ^? crResult . to _pactResult . _Right . _PLiteral . _LDecimal of
      Just d ->
        let actual = (fromRational (toRational d) :: Double)
            expected = 12020.670425990643 :: Double
            epsilon = 1e-9 :: Double
        in assertBool (msg <> ", actual = " <> show actual)
             (abs (actual - expected) < epsilon)
      Nothing -> assertFailure (msg <> ", expected decimal result")
